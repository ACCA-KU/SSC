...
class ISATconvolution(nn.Module):
    def __init__(self, in_node_feats, in_edge_feats, out_feats, activation, n_layers, dropout=0.2, batch_norm=False, residual_sum = False, alpha=0.1, max_dist = 4):
        ...
        self.init_h_func = nn.Sequential(
            nn.Linear(in_node_feats+in_edge_feats, out_feats, bias=True),
            nn.LeakyReLU(),
        )
        # Message Passing
        self.r2r = nn.ModuleList([DMPNNLayer(in_node_feats, out_feats, activation, dropout) for _ in range(n_layers)])
    ...
    def forward(self, graph, r_node, r2r_edge, i_node, d2d_edge):
        ...
        src_node_id = real_graph.edges()[0]
        hidden_feats = self.init_h_func(torch.cat([r_node[src_node_id], r2r_edge], dim=-1))
        inputs = hidden_feats
        for i in range(len(self.r2r)):
            hidden_feats = self.r2r[i](real_graph, r_node,inputs,hidden_feats)
            i_node = self.i2i[i](image_graph, i_node,)
        d_node1 = self.i2d(graph, i_node)
        d_node2 =  None

        real_graph.edata['feat'] = hidden_feats
        real_graph.ndata['feat'] = r_node
        real_graph.update_all(self.send_income_edge, self.sum_income_edge)
        h = dgl.sum_nodes(real_graph, 'hidden_feats')
        return h, d_node1, d_node2
