...
class SSCwoPEDMPNN(SSCMolecularNetwork):
    """SSC without polarity effects, using a directed MPNN solvent branch."""

    model_name = "SSCwoPE_DMPNN"
...
class ISATconvolution(nn.Module):
    def __init__(self, in_node_feats, in_edge_feats, out_feats, activation, n_layers, dropout=0.2, batch_norm=False, residual_sum = False, alpha=0.1, max_dist = 4):
        ...
        self.r2r = nn.ModuleList([
            DMPNNLayer(in_node_feats, in_edge_feats, out_feats, activation, dropout)
    ...
    def forward(self, graph, r_node, r2r_edge, i_node, d2d_edge):
        ...
        for i in range(len(self.r2r)):
            hidden_feats, direct_feats, backward_feats = self.r2r[i](
                real_graph,
                r_node,
                r2r_edge,
                direct_feats,
        ...
        h = graph_sum_pool(real_graph, hidden_feats)
