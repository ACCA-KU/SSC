...
from SSC.src.SSCnet.pyg_utils import (
    dot_to_real,
    expand_to_dot_nodes,
    graph_sum_pool,
    image_to_dot,
...
class SSC(SSCMolecularNetwork):
    """Solvent-aware SSC network with GAT branches."""
    ...
    def forward(self, **kargs):
        ...
        graph = inputs['graph']
        r_node = inputs['r_node']
        i_node = inputs['i_node']
        r_edge = inputs['r_edge']
        d_edge = inputs['d_edge']
        solv_graph = inputs['solv_graph']
        solv_node_feats = inputs['solv_node_feats']
        r_node = r_node.float()
