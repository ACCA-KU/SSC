...
from D4CMPP2.networks.src.AFP import AttentiveFP, Atom_AFPLayer
...
class SSCwoPEAFP(SSCMolecularNetwork):
    """SSC without polarity effects, using an AttentiveFP solvent branch."""

    model_name = "SSCwoPE_AFP"
    ...
    def __init__(self, config):
        ...
        self.embedding_solv_edge_lin = nn.Sequential(
            nn.Linear(config['edge_dim'], hidden_dim, bias=False)
        )
        ...
        self.GATs_solv = AttentiveFP(config)
    ...
    def forward(self, **kargs):
        ...
        solv_edge_feats = solv_edge_feats.float()
        solv_edge_feats = self.embedding_solv_edge_lin(solv_edge_feats)

        solv_r_graph = relation_graph(solv_graph, 'r_nd', 'r2r')

        r_node3, d_node1, d_node2 = self.ISATconv(graph, r_node2, r_edge, i_node, d_edge)

        solv_h,_ = self.GATs_solv(solv_r_graph, solv_node_feats, solv_edge_feats)
...
class ISATconvolution(nn.Module):
    def __init__(self, in_node_feats, in_edge_feats, out_feats, activation, n_layers, dropout=0.2, batch_norm=False, residual_sum = False, alpha=0.1, max_dist = 4):
        super().__init__()
        config = {
            'hidden_dim': in_node_feats,
            'conv_layers': n_layers,
            'dropout': dropout
        }
        # Message Passing
        self.r2r = nn.ModuleList([Atom_AFPLayer(config) for _ in range(n_layers)])
